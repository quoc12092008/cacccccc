(function() 
    ...
    function mmb()
        ...
        -- Function to get player data dynamically
        ...
        pcall(function()
            ...
            if playerData then
                if playerData:FindFirstChild("Level") then
                    playerData.Level.Changed:Connect(updateStats)
                end
                if playerData:FindFirstChild("Beli") then
                    playerData.Beli.Changed:Connect(updateStats)
                end
                if playerData:FindFirstChild("Fragments") then
                    playerData.Fragments.Changed:Connect(updateStats)
                end
                if playerData:FindFirstChild("Race") then
                    playerData.Race.Changed:Connect(updateStats)
                end
            end
        end)
        ...
        local function hasItem(itemName)
            local success, result = pcall(function()
                ...
                if playerData then
                    -- Check various Data folders
                    local folders = {"DevilFruit", "Inventory", "Swords", "Melees", "Accessories"}
                    for _, folder in ipairs(folders) do
                        local dataFolder = playerData:FindFirstChild(folder)
                        if dataFolder and dataFolder:FindFirstChild(itemName) then
                            return true
                        end
                    end
                end
        ...
        local function updateItems()
            pcall(function()
                -- Check GodHuman (also try variations)
                local hasGodHuman = hasItem("Godhuman") or hasItem("God Human") or hasItem("GodHuman")
                itemDots.godHuman.BackgroundColor3 = hasGodHuman and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)

                -- Check Cursed Dual Katana
                local hasCDK = hasItem("Cursed Dual Katana") or hasItem("Tushita")
                itemDots.cdk.BackgroundColor3 = hasCDK and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)

                -- Check Valkyrie Helm
                local hasValkyrie = hasItem("Valkyrie Helm") or hasItem("ValkyrieHelm")
                itemDots.valkyrieHelm.BackgroundColor3 = hasValkyrie and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)

                -- Check Soul Guitar
                local hasSoulGuitar = hasItem("Soul Guitar") or hasItem("SoulGuitar")
                itemDots.soulGuitar.BackgroundColor3 = hasSoulGuitar and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)

                -- Check Mirror Fractal
                local hasMirror = hasItem("Mirror Fractal") or hasItem("MirrorFractal")
                itemDots.mirrorFractal.BackgroundColor3 = hasMirror and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)

                -- Check Pull Lever (quest completion)
                local hasPullLever = false
                local playerData = player:FindFirstChild("Data")
                if playerData and playerData:FindFirstChild("QuestProgress") then
                    hasPullLever = playerData.QuestProgress:FindFirstChild("PullLever") and true or false
                end
                itemDots.pullLever.BackgroundColor3 = hasPullLever and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
            end)
        end

        -- Initial update for items
        task.spawn(updateItems)

        -- Auto-update items every 5 seconds
        task.spawn(function()
