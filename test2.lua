...
local function saveAllowedPetsToFile(petsData)
    local success, result = pcall(function()
        local filePath = "allowedPets.json"
        local jsonData = HttpService:JSONEncode({
            timestamp = os.time(),
            lastUpdate = os.date("%Y-%m-%d %H:%M:%S"),
            secret = petsData.secret or {},
            og = petsData.og or {},
            luckyBlocks = petsData.luckyBlocks or {},
            totalCount = #(petsData.secret or {}) + #(petsData.og or {}) + #(petsData.luckyBlocks or {})
        })
    ...
    if success then
        print("✅ Saved pets to workspace/allowedPets.json")
        return true
    else
        warn("❌ Failed to save pets: " .. tostring(result))
        return false
    end
end

local function loadAllowedPetsFromFile()
    local success, result = pcall(function()
        local filePath = "allowedPets.json"
        ...
        local petsData = HttpService:JSONDecode(fileContent)
        
        -- Check if file is older than 10 minutes
        local fileAge = os.time() - (petsData.timestamp or 0)
        if fileAge > 600 then -- 10 minutes
            print("⚠️  Pet list file is old (>10 min), will rescan")
            return nil
        end
    ...
    if success and result then
        print("✅ Loaded pets from file (updated: " .. (result.lastUpdate or "unknown") .. ")")
        return result
    else
        if result then
            warn("⚠️  Failed to load pets from file: " .. tostring(result))
        end
        return nil
    end
end
...
-- Hàm update allowed pets list (ĐỌC TỪ FILE TRƯỚC)
local function updateAllowedPetsList()
    local scannedPets = nil
    
    -- 1. Thử đọc từ file trước
    local fileData = loadAllowedPetsFromFile()
    if fileData then
        scannedPets = {
            secret = fileData.secret,
            og = fileData.og,
            luckyBlocks = fileData.luckyBlocks
        }
        print("📁 Using pets from file (total: " .. fileData.totalCount .. ")")
    else
        -- 2. Nếu không có file hoặc file cũ → Scan từ game
        print("🔍 Scanning pets from game...")
        scannedPets = scanPetsFromGame()
        
        if scannedPets then
            -- Lưu vào file để account khác dùng
            saveAllowedPetsToFile(scannedPets)
        end
    end
...
-- Hàm force rescan và cập nhật file (gọi mỗi 5 phút)
local function forceRescanAndUpdateFile()
    print("🔄 Force rescanning pets from game...")
    ...
    if scannedPets then
        ...
        print("✅ Pet list rescanned and file updated:")
        print("  🟡 Secret: " .. #scannedPets.secret)
        print("  🔵 OG: " .. #scannedPets.og)
        print("  🟣 Lucky Blocks: " .. #scannedPets.luckyBlocks)
...
local function startPetMonitor()
    ...
    print("╔════════════════════════════════════════════╗")
    print("║   AUTO PET SCANNER & TRACKER v4.1         ║")
    print("╠════════════════════════════════════════════╣")
    print("║ Player: " .. LocalPlayer.Name)
    print("║ Tracking: " .. #allowedPets .. " pets")
    print("║ Found: " .. #lastFoundPets .. " pets in plot")
    print("║ Auto-rescan: Every 5 minutes")
    print("║ File sync: workspace/allowedPets.json")
    ...
    recheckConnection = RunService.Heartbeat:Connect(function()
        ...
        -- Auto-rescan pets every 5 minutes (FORCE SCAN + UPDATE FILE)
        if currentTime - lastPetScanTime >= TIMING_CONFIG.petListRefreshInterval then
            forceRescanAndUpdateFile()
        end
