...
local TIMING_CONFIG = {
    petCheckInterval = 10, -- Giảm từ 30 xuống 10 để check nhanh hơn
    apiSendInterval = 60,
    forceUpdateInterval = 120,
    luckyBlockCheckInterval = 3, -- Giảm từ 5 xuống 3
    petListRefreshInterval = 5 * 60, -- 5 phút scan lại danh sách pet
    maxRetryOnFail = 5, -- Số lần retry khi đọc dữ liệu thất bại
    loopWaitTime = 1 -- Thời gian chờ giữa mỗi vòng lặp
}
...
-- Get animal list using Synchronizer với retry
local function getAnimalListViaSynchronizer()
    if not playerSync then return nil end
    
    for attempt = 1, TIMING_CONFIG.maxRetryOnFail do
        local success, animalList = pcall(function()
            return playerSync:Get("AnimalPodiums")
        end)
        
        if success and animalList and type(animalList) == "table" and next(animalList) then
            return animalList
        end
        
        if attempt < TIMING_CONFIG.maxRetryOnFail then
            task.wait(0.5) -- Chờ trước khi retry
        end
    end
...
-- Get animal list from plot (fallback method) với retry
...
-- Hàm đọc pet list với fallback tự động
local function getAnimalListSafe()
    -- Thử Synchronizer trước
    local animalList = getAnimalListViaSynchronizer()
    
    if animalList and type(animalList) == "table" and next(animalList) then
        return animalList, "Synchronizer"
    end
    
    -- Fallback sang Plot method
    warn("⚠️ Synchronizer failed, trying Plot method...")
    animalList = getAnimalListViaPlot()
    
    if animalList and type(animalList) == "table" and next(animalList) then
        return animalList, "Plot"
    end
...
local function detectLuckyBlock()
    local animalList, method = getAnimalListSafe()
...
local function getAllowedPetsFromSynchronizer()
    ...
    if not animalList then 
        warn("⚠️ Cannot read pet data - will retry next cycle")
        return pets 
    end
    
    -- Debug log khi dùng fallback
    if method == "Plot" then
        print("📋 Using Plot fallback method")
    end
...
local function startPetMonitor()
    ...
    print("║ FPS-Safe: Works at any FPS (even 1fps) ✓")
    print("╚════════════════════════════════════════════╝")
    ...
    -- Spawn lucky block checker
    task.spawn(checkAndOpenLuckyBlocks)
    
    -- Main monitoring loop - sử dụng while loop thay vì Heartbeat
    monitorTask = task.spawn(function()
        while getgenv().PET_TRACKER_RUNNING do
            ...
            if currentTime - lastPetCheckTime >= TIMING_CONFIG.petCheckInterval then
                ...
                -- Nếu không đọc được dữ liệu và có pet cũ, bỏ qua lần này
                if #newPets == 0 and #lastFoundPets > 0 then
                    warn("⚠️ Failed to read pets, skipping this check cycle")
                    task.wait(2) -- Chờ 2 giây trước khi retry
                else
            ...
            task.wait(TIMING_CONFIG.loopWaitTime)
        end
